...
import { a as invoke, r as Resource } from "./tauri-apps__api.mjs";
//#region node_modules/@tauri-apps/plugin-fs/dist-js/index.js
var dist_js_exports = /* @__PURE__ */ __exportAll({
...
async function writeTextFile(path, data, options) {
	if (path instanceof URL && path.protocol !== "file:") throw new TypeError("Must be a file URL.");
	const encoder = new TextEncoder();
	await invoke("plugin:fs|write_text_file", encoder.encode(data), { headers: {
		path: encodeURIComponent(path instanceof URL ? path.toString() : path),
		options: JSON.stringify(options)
	} });
}
