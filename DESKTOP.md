...
- Cold 100k: tree/editor in **seconds**. Title search for the open window (`ready-meta`) should land before the rest of the folder is listed, and should not grow like a full listing. Heads for those open notes follow, then the banner reaches Ready. The remaining names keep listing in the background and must not block open, scroll, graph, or title search. A title the listing has not reached yet is missing until that batch lands. A body word in a note you have not opened stays missing until you open it. Do **not** claim SCALE READY from one soak.
