...
| `ready-fts-partial` | Titles for the vault, deep heads for the open notes | No |
...
| Layer | What |
|--------|------|
| Rust | `vault_index_fill_from_disk` is **async** (blocking pool) and **phased**: walk → **title/path FTS seed** (priority folder + Hub-named files first, ~2k rows) → **`ready-meta` immediately** → deep heads for a stable open window (priority paths, otherwise the start of the walk), at most `EAGER_CONTENT_CAP` files. A later fill reads that same window again only where it is still shallow. It does **not** read a body for every remaining note. The open page may already have a short peek during the walk, capped once a few hundred notes already have a head. It does **not** write 100k empty-body FTS rows before search is live. Incremental: skip notes whose path+mtime+size already match at that `fill_depth`. While writing a head it extracts `[[wikilinks]]` into `link_edge`. A warm FTS index filled before that path gets a one-shot backfill from `note_fts` bodies (no JS hydrate). Emits `ready-meta` / `ready-fts-partial` (open window) / `done`. `done` on a large vault stays `ready-fts-partial`: titles plus the notes that were opened. Dedicated writer connection so the UI/search mutex is not held. A second fill for the same DB **joins** the in-flight job (never `already running` as a user-visible failure). UI connections use a 15s busy timeout; close/wipe/rebuild wait out an in-flight fill. PASSIVE WAL checkpoint only (no TRUNCATE). IDs via `desk_node_id`. `vault_index_list_links` returns grouped edges for the JS link index. Opening a note after fill writes that note's deep head into SQLite. |
