...
- Title search for the first page is announced before the rest of the folder is listed. Folders are visited in name order, so the top of the vault (including `00-Inbox`) is in that page. That page stays the same size as the folder grows: Ready reads only enough of the first directory to fill the page (it does not read every name there first), it does not stat every file in that directory, and it does not load the catalog into memory. The banner does not wait to count every file, and it does not show the full vault at 100%. Reopening reads a stored total when the listing has finished before; it does not count every row to paint that page, and a page that already appeared is not reported as a folder-permission failure. The rest of the names are listed afterward, yielding between batches and checkpointing so search does not slow down as the listing gets longer. A title in a folder the listing has not reached yet is missing until that batch lands. Note text is still only read for the open window. A word in a note you have not opened is still missing, and so is a word past the deep head of a note you have opened.
- Measured in the fill harness, cancelled at the first Ready page, not in the desktop window. A catalog of 500,000 rows reached that page in 4 ms against 2 ms with no extra rows. Official-shaped trees of 8,000 and 80,000 files reached it in 2 ms and 3 ms; the first folder there is a slice of the vault, which is the stand-in for a half-million-note official layout. One directory of 40,000 names took 88 ms (2,000 names took 6 ms) and still returned the first page. A half-million-file tree was not opened. Background listing still visits every file.
