...
- After titles are searchable, desktop reads note text for a fixed window: the open notes, at most a few hundred files. A later open reads that same window again only where it is still shallow. It does not continue through the rest of the vault. Opening a note indexes that note. A word in a note you have not opened is still missing, and so is a word past the deep head of a note you have opened. The first page may already have had a short peek during the walk. A 500,000-note open is not claimed here.
