...
- Title search for the first page is announced before the rest of the folder is listed. Folders are visited in name order, so the top of the vault (including `00-Inbox`) is in that page. Ready is that page: the banner does not wait to count every file, and it does not show the full vault at 100%. The rest of the names are listed afterward and yield between batches, so open, scroll, and the graph do not wait for the listing to finish. A title in a folder the listing has not reached yet is missing until that batch lands. Note text is still only read for the open window. A word in a note you have not opened is still missing, and so is a word past the deep head of a note you have opened. A 500,000-note open is not claimed here.
