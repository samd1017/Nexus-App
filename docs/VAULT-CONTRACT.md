...
- The open walk indexes a head of each file so the window can paint. The rest of the note is indexed afterward, up to a fixed ceiling, and the text is not kept. A link or word past that ceiling is still missing. A small folder shows every note and searches an in-memory index. Opening a note puts the rest of that note into the index, up to the same ceiling, including a rare word that contains digits.
